...
<body>
	...
	<div class="d2" style="margin-left:25%; padding: 20px 40px 40px 100px; border-left:2px solid #FFD700;">
		<?php
			...
			echo "<h2 class='h01'>". $menu." Menu</button></h2>";
			$sql = "SELECT * FROM final_menu
					WHERE Dish_menu = '$menu'
					";
			...
			$result=mysqli_query($conn,$sql);
				if(mysqli_num_rows($result)>0){
					...
					echo "<tr><td class='tt2'>" .$rows["Dish_name"]."</td><td class='tt2'>" .$rows["Dish_category"]. "</td><td class='tt2'>" .$rows["Dish_price"]. "</td><td class='tt2'>" .$rows["Description"]."</td><td class='tt2'><a href='dash_edit.php?id= $rows[Dish_id]&menu=$rows[Dish_menu]&item=$rows[Dish_name]&price=$rows[Dish_price]&cat=$rows[Dish_category]&descp=$rows[Description]'><button>Edit</button></a></td></tr>";
					}
					echo "</table>";
				}
				else{
